...
type Config struct {
	...
	RS256PrivateKeyPath string        `yaml:"private_key_path" env:"RS256_PRIVATE_KEY_PATH"`
...
func MustLoad() *Config {
	...
	if cfg.RS256PrivateKeyPath == "" && cfg.Env == "production" {
		panic("private key path is required in production environment")
	}

	if _, err := os.ReadFile(cfg.RS256PrivateKeyPath); err != nil {
		if cfg.Env == "production" {
			panic("private key is empty or file not exists")
		}

		log.Println("Incorrect RS256 key, using empty path")
		cfg.RS256PrivateKeyPath = ""
	}
